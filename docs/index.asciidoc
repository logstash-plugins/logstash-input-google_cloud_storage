...
the Logstash Ruby gem's cache and then proceed with the usual installation of the plugin with
the `logstash-plugin` install tool
...
Then use the plugin installation tool:
[source,bash]
----------------------------------
